...
- **result**: 🔴 **本项目对 rule_v4 的第一次真实提升**。把"位置采样的分布"从位置网换成
  ...
  **② 有了好先验，k 从 24 缩到 8 没有可测代价**（34.4% vs 39.1%，p=0.69）—— 用户
  "训好先验就能缩小候选集、从而让搜索变便宜"的前提，在强对手上被验证。

  **⚠️ 三条限制（必须一起读）**：
  1. **价格**：价值先验目前是**暴力算**的（根节点每个出招回合枚举钉类全部合法格并过价值网，
     ~270 次前向）⇒ 它**现在更贵**，不是更便宜。收益是**信息**（"好先验下 k=8 就够"），
     成本收益要等先验由**一次前向**（训练出来的网络）给出才兑现 —— 那正是计划的下一步。
  2. **一个混杂未排除**：该回调枚举的格子额外过了一遍 `can_apply_operation` 过滤（保证提出的
     动作可执行），而位置网采样不保证这点 ⇒ 部分增益可能来自"少浪费回合"而不是"位置更好"。
     对照臂 `--pos-prior uniform`（同样枚举+过滤、权重全相同）正在 `value_prior_verify` 里跑。
