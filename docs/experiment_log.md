...
既然类头和价值头**都**产不出好动作，好动作只能来自**外部**。文档里已有的证据是
`docs/distill_rule_v4.md`：**从 rule_v4 蒸馏到 58-62% vs rule_v4**——这是唯一一次超过 50%，
而整个 AZ 自对弈线**从未**超过 50%。⇒ "蒸馏 / 把 rule_v4 放进对手池"可能比
"在 AZ 自我循环里挤好动作"更接近答案。
