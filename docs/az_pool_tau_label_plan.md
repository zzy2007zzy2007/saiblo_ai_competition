...
### tau 扫描：加大 tau 消不掉捷径（2026-09-12 实测）

递推 `raw_t = (d_t + γd_{t+1} + γ²d_{t+2} + …)/(1+γ+γ²+…)`（γ=exp(-1/tau)）里**当前帧
是第一项**，所以 tau→0 时标签→d_t（捷径 100%），tau→∞ 时→余下整局平均（当前权重 1/N）。
方向上加大 tau 应该减弱捷径——实测确实减弱，但**极慢**，且**代价是逐帧信号变弱**：

| tau | corr | R²（可被抄当前血量差解释）| 局内逐帧 std |
|-----|------|------------------------|-------------|
| 5 | 0.994 | 0.988 | 0.277 |
| 20 | 0.972 | 0.943 | 0.261 |
| **50** | **0.937** | **0.871** | **0.234** |
| **100** | **0.898** | **0.791** | **0.204** |
| 200 | 0.856 | 0.705 | 0.171 |
| 1000（≈整局平均）| 0.796 | 0.594 | 0.127 |

**两头堵**：捷径主要来自血量差轨迹的自相关（不是加权窗口），所以 tau=1000 时 R² 仍有
0.59；而加大 tau 又把局内逐帧 std 从 0.234 压到 0.127——恰好毁掉我们需要的逐帧信号。

**结论**：tau 不是这个问题的解，**`rel` 才是**（模式设计上直接减掉 d_t，corr −0.24）。
因此按用户要求补跑了 tau=100 全 6 臂（`_tmp_pooltau100.sh`）作为 tau 轴的第二点，
用于**实证确认**"tau 无效"，而不是期待翻盘。

### 换权重 `w_k = k·γ^k`：比调 tau 更对症（2026-09-12 实测）

用户提议把未来偏移 k 的权重从 `γ^k` 换成 `k·γ^k`。注意 **k=0 时权重恰为 0** ——
当前帧被完全排除，权重峰值移到 k≈tau。实测（`_tmp_weight_family_corr.py`）：

| 权重 | tau | corr | R² | 局内逐帧 std |
|------|-----|------|-----|-------------|
| `γ^k`（现状）| 50 | 0.937 | 0.871 | 0.234 |
| **`k·γ^k`** | **50** | 0.863 | **0.723** | **0.212** |
| `γ^k` | 100 | 0.898 | 0.791 | 0.204 |
| **`k·γ^k`** | **100** | 0.802 | **0.606** | **0.172** |
| `rel` | 50 | −0.241 | ≈0 | 0.147 |

**关键结论：`k·γ^k` 在取舍曲线上严格优于调 tau。**
- 用 `γ^k` 把 R² 降到 ≈0.72 需要 tau≈**200**，代价是逐帧 std 掉到 **0.171**；
  用 `k·γ^k` 在 tau=**50** 就做到 R² **0.723** 且逐帧 std 保留 **0.212**（两轴同时更优）。
- tau=100 同理：kgeo(0.606/0.172) 优于 geo≈1000(0.594/0.127)。

即"**把当前帧权重置零**"比"拉长加权窗口"更对症——符合机制分析（捷径来自当前帧的
直接可读性，而非窗口长度）。

**但它没有消灭捷径**（R² 0.72 仍高）。接近零的只有 `rel`（−0.24）；且 `rel` tau=100 的
逐帧 std(0.182) 与 kgeo tau=100(0.172) 相当，只是幅度被压小（|label| 0.15 vs 0.35，
可调 label-scale）。
