...
| M1e | 接入 bundle MCTS（鸭子类型门面） | 自对弈采集用上 C++ 引擎 | ✅ 门面完成，搜索 **2.2×** |

## 9. 门面 + 优化结果（2026-08-08 更新）

- **GameStateFacade**（鸭子类型）：接口与 GameState 完全一致，AI 代码零改动，仅创建处需换 `GameStateFacade.initial`。缓存派生视图（修掉信息素重复转换）。
- **类采样向量化**：每头每扩展算一次类分布 + 批量 rng.choice，decode_head 6×，32/深度4 搜索 2.6×。
- **关键发现：deep_clone 成本随状态大小增长**（中盘 1.9ms vs 开局 21μs）→ 门面 can_apply 的克隆探测在中盘不可用（14K 次 = 12s/决策）。
- **can_apply_dry**（无克隆）：完整冷路径模拟（金币/塔数/used_tower/camp/冷却），与克隆探测 90 回合 0 差异，中盘 2.2μs（380×）。
- **最终：128/深度4 搜索 Python 6.70s → C++ 门面 3.03s = 2.2×**。
