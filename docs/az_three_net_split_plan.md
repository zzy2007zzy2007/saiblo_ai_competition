...
### 判据复核（搜索对搜索镜像）——两个判据同向，位置线到此见顶

台子性质：两侧用**同一全局回合号**定搜索 seed ⇒ 策略相同时对局逐位相同 ⇒ **null 臂恰好
1.0000 / SE=0**（像 raw 镜像一样有零方差对照，但测的是部署真正跑的搜索）。

| 轮 | 搜索镜像（t）| raw 镜像（t）|
|---|---|---|
| 1 | 1.0312 (+0.39) | 1.1406 (+1.49) |
| 2 | **1.1094 (+1.19)** | **1.2969 (+3.60)** |
| 3 | 1.0781 (+1.00) | 1.0000 (0.00) |
| 4 | 1.0469 (+0.60) | 1.0469 (+0.57) |
| 5 | **0.9688 (−0.35)** | 1.0312 (+0.34) |

**两条曲线同向：第 2 轮见顶后衰减，第 5 轮搜索侧低于 1.0；搜索侧无一显著点。**
⇒ "raw 镜像测错东西"的假设作废；第 4 轮 rule_v4=31.2%（p=0.13）判为噪声。

**位置线的定论（就现有分辨率）**：位置网训练**能**改变行为（Phase 2 单轮 raw 镜像 t=5.29
是今天唯一显著的训练效应），但
1. **不可靠复现**（循环第 1 轮同管线、仅换 400 局游戏 → 不显著）；
2. **不复利**（两条曲线都在第 2 轮见顶后衰减，我的"跨轮累积策略目标"设计错误是可疑主因）；
3. **搜索侧测不到**（部署时跑的正是搜索，所以这一条最要紧）；
4. 分辨率限制：64 对配对下 ±16pp（2σ），**不能排除真存在 +10pp 级别的效应**——只是证不出来。

**下一步的前提**（若要继续）：修循环的两个设计错误（每轮只用自己的数据、每轮放大到
1500-2000 局）；**或**转去做"随机钉类"给位置网铺通道覆盖（为类轴修好那天准备）。
