...
class GALoggingSubsystem:
    ...
    def log_generation(
        ...
        duration: float,
        num_benchmark_agents_used: Optional[int] = None,
        benchmark_kendall_tau_final: Optional[float] = None,  # 已弃用，仅供向后兼容
        illegal_stats: Optional[Dict[str, int]] = None,
    ) -> None:
        """记录每代的日志"""
        # 1. 控制台输出
        logger.info(
            f"Generation {generation}: "
            f"best_rating={population.best_rating:.1f}, "
            f"mean_rating={population.mean_rating:.1f}, "
            f"diversity={population.diversity:.4f}, "
            f"duration={duration:.1f}s"
        )
        if illegal_stats:
            touchstone_illegal = illegal_stats.get("touchstone_illegal", 0)
            rr_illegal = illegal_stats.get("round_robin_illegal", 0)
            baseline_illegal = illegal_stats.get("baseline_illegal", 0)
            total_illegal = touchstone_illegal + rr_illegal + baseline_illegal
            avg_rounds = illegal_stats.get("avg_rounds", 0)
            touchstone_avg = illegal_stats.get("touchstone_avg_rounds", 0)
            rr_avg = illegal_stats.get("round_robin_avg_rounds", 0)
            baseline_avg = illegal_stats.get("baseline_avg_rounds", 0)
            logger.info(
                f"Generation {generation} illegal actions: "
                f"touchstone={touchstone_illegal}, "
                f"round_robin={rr_illegal}, "
                f"baseline={baseline_illegal}, "
                f"total={total_illegal}, "
                f"avg_rounds={avg_rounds} "
                f"(touchstone={touchstone_avg}, rr={rr_avg}, baseline={baseline_avg})"
            )

        # 2. 保存种群统计 JSON
        self._save_population_stats(
            generation, population, seeds,
            num_benchmark_agents_used=num_benchmark_agents_used,
            benchmark_kendall_tau_final=benchmark_kendall_tau_final,
        )
    ...
    def _save_population_stats(
        self,
        generation: int,
        population: Population,
        seeds: List[Individual],
        num_benchmark_agents_used: Optional[int] = None,
        benchmark_kendall_tau_final: Optional[float] = None,
    ) -> None:
        """保存种群统计到 JSON"""
        ...
        stats = {
            ...
            "num_seeds": len(seeds),
            "seed_rating_range": (
                seeds[-1].trueskill_mu if seeds else 0.0,
                seeds[0].trueskill_mu if seeds else 0.0,
            ),
        }

        if num_benchmark_agents_used is not None:
            stats["num_benchmark_agents_used"] = num_benchmark_agents_used
        if benchmark_kendall_tau_final is not None:
            stats["benchmark_kendall_tau_final"] = benchmark_kendall_tau_final
    ...
    def _save_battle_results(self, generation: int, population: Population, seeds: List[Individual]) -> None:
        ...
        rankings = [
            {"id": ind.id, "trueskill_mu": ind.trueskill_mu, "seed_rank": ind.seed_rank}
            for ind in sorted(population.individuals, key=lambda x: x.trueskill_mu, reverse=True)
        ]

        seed_list = [
            {"id": s.id, "trueskill_mu": s.trueskill_mu, "rank": s.seed_rank, "parents": s.parent_ids}
            for s in seeds
        ]
    ...
    def _write_tensorboard(self, generation: int, population: Population, baseline_results: Dict) -> None:
        ...
        try:
            ...
            writer.add_scalar("ga/best_rating", population.best_rating, generation)
            writer.add_scalar("ga/mean_rating", population.mean_rating, generation)
