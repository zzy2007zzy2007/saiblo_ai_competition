...
# 每回合单 agent 最多执行的动作数（硬上限，防止无限循环）
...
def _collect_multi_ops(agent, state, max_actions=MAX_ACTIONS_PER_TURN):
    """在一个回合内多次调用 agent.choose_operations()，累积多操作。

    每次 agent 返回一个操作后立即 apply 到 state，使后续的
    action_mask 和 observation 反映最新状态。
    agent 返回 [] (NOOP) 或操作非法时终止循环。

    Returns:
        累积的 Operation 列表（已在 state 上执行）。
    """
    ops = []
    for _ in range(max_actions):
        new_ops = agent.choose_operations(state)
        if not new_ops:                          # NOOP → 本回合结束
            break
        valid = _filter_valid_operations(new_ops, state, agent.player_id)
        if not valid:                            # 非法操作 → 终止
            # 非法计数由 agent 内部记录（illegal_action_count）
            break
        ops.extend(valid)
        state.apply_operation_list(agent.player_id, valid)
        if state.terminal:
            break
    return ops
...
def _execute_battle(
    ...
    while not state.terminal and round_count < max_rounds:
        try:
            ...
            # ── 收集并执行多动作（先手 → 后手，循环内立即 apply）──
            illegal_before_1 = agent1.illegal_action_count
            illegal_before_2 = agent2.illegal_action_count

            if first_player == 0:
                ops_1 = _collect_multi_ops(agent1, state)
                if not state.terminal:
                    ops_2 = _collect_multi_ops(agent2, state)
                else:
                    ops_2 = []
            else:
                ops_2 = _collect_multi_ops(agent2, state)
            ...
            illegal_this_round_1 = agent1.illegal_action_count - illegal_before_1
            illegal_this_round_2 = agent2.illegal_action_count - illegal_before_2
