...
class BasicRandomAI(AntWarAgent):
    ...
    def choose_operations(self, state) -> List[Any]:
        ...
        mask = self._mask_handler.get_action_mask(state, self.player_id)
        valid = np.where(mask > 0.5)[0]
        if len(valid) > 0:
            action_id = int(np.random.choice(valid))
            op = self._mask_handler._action_id_to_op(action_id, state, self.player_id)
...
__all__ = ["BasicRandomAI", "BasicTowerAI", "MediumRuleAI"]
