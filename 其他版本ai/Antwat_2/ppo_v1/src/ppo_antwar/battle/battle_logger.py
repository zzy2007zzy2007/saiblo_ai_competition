...
class BattleLogger:
    ...
    支持两种初始化方式：
    1. 传入 PathConfig 对象（推荐）
    2. 传入 log_dir 字符串路径（向后兼容）
    """
    ...
    def __init__(self, agent1_name: str, agent2_name: str,
        ...
        if path_config is not None:
            ...
            self._path_config = PathConfig(base_dir=log_dir)
        else:
            raise ValueError("Either path_config or log_dir must be provided for BattleLogger")
    ...
    def _create_log_dir(self) -> str:
        ...
        dir_path = self._path_config.battle_dir / dir_name
    ...
    def _configure_logger(self):
        ...
        handler_id = logger.add(
            os.path.join(self._log_dir, 'battle_debug.log'),
            format=log_format,
            level="DEBUG",
            enqueue=True,
            serialize=False
        ...
        handler_id = logger.add(
            os.path.join(self._log_dir, 'battle_info.log'),
            format=log_format,
            level="INFO",
            enqueue=True,
            serialize=False
        ...
        handler_id = logger.add(
            os.path.join(self._log_dir, 'battle_warning.log'),
            format=log_format,
            level="WARNING",
            enqueue=True,
            serialize=False
        ...
        handler_id = logger.add(
            os.path.join(self._log_dir, 'battle_error.log'),
            format=log_format,
            level="ERROR",
            enqueue=True,
            serialize=False
        ...
        handler_id = logger.add(
            os.path.join(self._log_dir, 'battle_all.log'),
            format=log_format,
            level="DEBUG",
            enqueue=True,
            serialize=False
    ...
    def debug(self, category: str, message: str):
        if self.log_level.get_level_number() <= LogLevel.DEBUG.get_level_number():
            logger.bind(category=category).debug(message)

    def info(self, category: str, message: str):
        if self.log_level.get_level_number() <= LogLevel.INFO.get_level_number():
            logger.bind(category=category).info(message)

    def warning(self, category: str, message: str):
        if self.log_level.get_level_number() <= LogLevel.WARNING.get_level_number():
    ...
    def timing_stop(self, operation: str, duration: float = None) -> float:
        ...
        if duration > self.warning_threshold:
            self.warning('timeout', f"{operation} took {duration:.2f}s (exceeds {self.warning_threshold}s)")
    ...
    def log_gpu_downgrade(self, message: str):
        """记录 GPU 降级警告"""
        self.warning('gpu', f"GPU DOWNGRADE: {message}")
