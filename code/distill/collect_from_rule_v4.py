"""Collect behavioral cloning data from rule_v4 AI for distillation.

rule_v4 produces more diverse non-HOLD actions than ExampleAI (builds
and upgrades towers at edge positions), so data from this teacher helps
the model learn state-dependent behavior beyond just HOLD.

Usage:
    python code/distill/collect_from_rule_v4.py --out-dir distill_data_rule_v4 --games 500 --workers 16
"""
...
def collect_game(seed: int, max_actions: int = 9999) -> list[dict]:
    """Play one game, record rule_v4's actions as hard labels + soft scores.
    ...
    """
    # Import rule_v4 AI
    rule_v4_path = Path(__file__).resolve().parents[1] / "其他版本ai" / "rule_v4"
    sys.path.insert(0, str(rule_v4_path))
    from ai import RuleBasedAI
    ...
    ai = RuleBasedAI(seed=seed)
    opp = RuleBasedAI(seed=seed + 9999)
    ...
    for turn in range(MAX_ROUND):
        ...
        # ── 1. Get rule_v4's chosen action ──────────────────────────
        bundles = catalog.build(state, our_player, rerank=False)

        # Use rule_v4's own bundle selection
        best = ai.choose_bundle(state, our_player, bundles)

        # Compute soft targets from all bundles
...
def _worker(seed: int, out_dir: str) -> dict:
    ...
    class_scores = np.stack([t["class_scores"] for t in turns], axis=0)
...
def main():
    parser = argparse.ArgumentParser(description="Collect BC data from rule_v4 AI")
    parser.add_argument("--out-dir", type=str, default="distill_data_rule_v4",
    ...
    print(f"Collecting {args.games} games (rule_v4), {args.workers} workers → {out_dir}")
    t0 = time.time()
