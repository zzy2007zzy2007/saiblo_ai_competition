...
def _worker(args: tuple) -> dict:
    ckpt_path, iterations, max_depth_rounds, seed, our_player, opponent, hotstart, no_search, select_by_prior, intent_decoding, one_head, bundle_mcts, depth0, value_tanh, k, c_puct, t_class, t_pos, self_raw_opponent, max_rounds, native_engine = args
    import torch
    ...
    if no_search:
        ...
        _, net_fn = make_net_fn_from_ckpt(ckpt_path, feat, value_tanh=value_tanh)
        if depth0:
...
def main() -> None:
    ...
    jobs = [
        (args.checkpoint, args.iterations, args.max_depth_rounds, args.seed + s,
         ...
         not args.no_value_tanh, args.k, args.c_puct,
