...
#       [data_dir] [ckpt_dir] [games] [workers] [t_class] [t_pos] [k] [value_passes]
set -u
...
prev="$INIT"
echo "[run] batches=$N_BATCHES start=$START_BATCH init=$prev iters=$ITERATIONS native=$NATIVE games=$GAMES workers=$WORKERS data=$DATA ckpt=$CKPT_DIR t_class=$T_CLASS t_pos=$T_POS k=$K value_passes=$VALUE_PASSES"
for b in $(seq "$START_BATCH" $((START_BATCH + N_BATCHES - 1))); do
    echo "==================== [batch $b] 开始 ===================="
    BATCH_DIR="$DATA/batch$b"
    NATIVE_ARG=""
    if [ "$NATIVE" = "1" ]; then NATIVE_ARG="--native-engine"; fi
    echo "[batch $b] 采集 $GAMES 局 ($ITERATIONS iter / depth 4, $WORKERS workers) seed=$b -> $BATCH_DIR [native=$NATIVE, t_class=$T_CLASS, t_pos=$T_POS, k=$K]..."
    "$PY" code/my_ai/az_intent/az_selfplay.py \
        ...
        --t-class "$T_CLASS" --t-pos "$T_POS" --k "$K"
